...
void X86InstrInfo::storeRegToStackSlotOrFPU(
  ...
  assert(MFI.getObjectSize(FrameIdx) >= TRI->getSpillSize(*RC) &&
         "Stack slot too small for store");
  LLVM_DEBUG(dbgs() << "Spilling to stack slot " << FrameIdx << "\n");
  if (RC->getID() == X86::TILERegClassID) {
    LLVM_DEBUG(dbgs() << "TILERegClas"
                      << "\n");
    unsigned Opc = X86::TILESTORED;
    // tilestored %tmm, (%sp, %idx)
    Register VirtReg = RegInfo.createVirtualRegister(&X86::GR64_NOSPRegClass);
    ...
    unsigned Alignment = std::max<uint32_t>(TRI->getSpillSize(*RC), 16);
    LLVM_DEBUG(dbgs() << Alignment << " byte spill"
                      << "\n");
    if (FrameIdx >= 16 || !SPILL_TO_FPU) {
      LLVM_DEBUG(dbgs() << "FrameIdx >= 8"
                        << "\n");
      bool IsAligned =
          (Subtarget.getFrameLowering()->getStackAlign() >= Alignment) ||
          (RI.canRealignStack(MF) && !MFI.isFixedObjectIndex(FrameIdx));
      ...
      LLVM_DEBUG(dbgs() << "Using FP Reg " << FrameIdx << "\n");
      LLVM_DEBUG(dbgs() << "Instr: ");
      LLVM_DEBUG(MI->dump());
      LLVM_DEBUG(dbgs() << "Spill size: " << TRI->getSpillSize(*RC) << "\n");
      MCRegister DestReg = X86::XMM0 + FrameIdx;
      unsigned Opc = // used to be VMOVPDI2DIrr, VMOV64toPQIrr
          TRI->getSpillSize(*RC) == 4 ? X86::VMOVDI2PDIrr : X86::VMOV64toPQIrr;
...
void X86InstrInfo::loadRegFromStackSlotOrFPU(
  ...
  if (RC->getID() == X86::TILERegClassID) {
    ...
    if (FrameIdx >= 16 || !SPILL_TO_FPU) {
      ...
      // Spill to FPU
      LLVM_DEBUG(dbgs() << "Reloading from FP Reg " << FrameIdx << "\n");
      LLVM_DEBUG(dbgs() << "Spill size: " << TRI->getSpillSize(*RC) << " bytes"
                        << "\n");
      unsigned Opc = // used to be VMOV64toPQIrr
          TRI->getSpillSize(*RC) == 4 ? X86::VMOVPDI2DIrr : X86::VMOVPQIto64rr;
      MCRegister SrcReg = X86::XMM0 + FrameIdx;
      BuildMI(MBB, MI, DebugLoc(), get(Opc), DestReg).addReg(SrcReg, RegState::Kill);
    }
  }
}
